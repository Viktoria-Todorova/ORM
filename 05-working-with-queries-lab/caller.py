...
def find_authors_nationalities():
    #authors = Author.objects.all().filter(nationality__isnull=False)
...
def filter_authors_by_nationalities(nationality:str):
    ...
    return "\n".join(a.biography if a.biography is not None
                     else f"{a.first_name} {a.last_name}"
                     for a in filtered_authors)


#print(filter_authors_by_nationalities(1,2))


#6
def filter_authors_by_birth_year(y1:int,y2:int):
    filtered_authors = Author.objects.all().filter(birth_date__year__range=(y1, y2)).order_by("-birth_date")

    return "\n".join(f"{a.birth_date}: {a.first_name} {a.last_name}" for a in filtered_authors)
...
#print(change_reviewer_name(reviewer_name="", new_name=""))