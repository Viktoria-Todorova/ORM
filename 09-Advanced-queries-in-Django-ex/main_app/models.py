...
from main_app.maangers import RealEstateListingManager, VideoGameManager
...
class VideoGame(models.Model):
    ...
    release_year = models.PositiveIntegerField(validators=[RangeValidator(
                                     min_value=1990,
                                     max_value=2023,
...
class Invoice(models.Model):
    ...
    def get_invoice_with_billing_info(cls,invoice_number: str) -> QuerySet['Invoice']:
        """
        SELECT * FROM invoice
        JOIN billing_info ON billing_info_id =biling_info.id
        WHERE invoice_number= argument
...
class Task(models.Model):
    ...
    def search_tasks(cls,query: str)-> QuerySet['Task']:
        """
        SELECT * FROM tasks WHERE title ILIKE '%query%'
        """
    ...
    def recent_completed_tasks(cls,days: int)-> QuerySet['Task']:
        return cls.objects.filter(is_completed=True,
                                  completion_date__gt=F('creation_date') - timedelta(days=days))
...
class Exercise(models.Model):
    ...
    def get_exercises_with_difficulty_and_repetitions(cls,min_difficulty: int, min_repetitions: int) ->QuerySet['Exercise']:
        ...
        # return cls.objects.filter(
        #     Q(difficulty_level = min_difficulty) & Q(repetitions__gte = min_repetitions)
        # )
