...
class RealEstateListingManager(models.Manager):
    ...
    def popular_locations(self):
        ...
        ).order_by('-location_count','location')[:2] #limit 2
...
class VideoGameManager(models.Manager):
    ...
    def recently_released_games(self,year: int) -> QuerySet['VideoGame']:
        return self.filter(release_year__gt=year)

    def highest_rated_game(self)-> QuerySet['VideoGame']:
        """"
        SELECT * FROM vide_game
        ORDER BY rating DESC
        LIMIT 1
        """
        return self.order_by('-rating').first()

    def lowest_rated_game(self)-> QuerySet['VideoGame']:
        return self.order_by('rating').first()

    def average_rating(self)-> str:
        average_rating = self.aggregate(average_rating = Avg('rating'))['average_rating'] #returns {average_ration : ''}
