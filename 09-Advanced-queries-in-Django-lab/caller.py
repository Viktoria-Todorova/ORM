...
def ordered_products_per_customer():
    ...
    for order in orders:
        ...
        for ordered_products in order.orderproduct_set.all():
            result.append(f'- Product: {ordered_products.product.name}, '
                          f'Category: {ordered_products.product.category.name}')
...
#5
def give_discount():
    # query =Q(is_available=False) & Q(price__gt=3.00)
    # products_to_be_discounted = Product.objects.filter(query)
    # products_to_be_discounted.update(price=F('price') * 0.7)
    Product.objects.filter(is_available=True, price__gte=3.00).update(price=F('price') * 0.7)
